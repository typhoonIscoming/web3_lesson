...
这是Solidity中最容易混淆的地方。uint[3][4]表示：

4个数组
* 每个数组长度为3
* 从右向左读更容易理解：[4]表示4个，[3]表示长度为3的数组。

如果你想要3行4列的矩阵，应该声明为uint[4][3]。

## **Q3：为什么要限制数组最大长度？**

限制数组大小是关键的安全措施：

* 防止Gas耗尽：大数组遍历可能超过区块gas限制
* 保证可用性：确保所有函数都能正常执行
* 防止DoS攻击：恶意用户不能通过填满数组使合约失效
* 用户体验：用户可以预估交易成本
...
* 内存分配：Memory是栈式分配，创建时就分配固定空间
* 性能考虑：动态扩展memory数组会很低效
* 设计简化：简化EVM实现
